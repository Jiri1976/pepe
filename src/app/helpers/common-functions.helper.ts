...
export function maxLenValidator(max: number, message: string) {
  return ({ value }: { value: () => string }) =>
    value().length > max ? { kind: 'maxLength', message } : null;
}
...
export function destinationForUserSignal(user: User) {
  const isFM = user.destinations.some(
    (dest) =>
      dest.destination === 'F-M' &&
      dest.positions &&
      dest.positions?.length > 0,
  );
  const isOVA = user.destinations.some(
    (dest) =>
      dest.destination === 'OVA' &&
      dest.positions &&
      dest.positions?.length > 0,
  );
  if (isFM && !isOVA) {
