...
export const WarehouseStore = signalStore(
  ...
  withMethods((store) => {
    ...
    const getWarehouseCards = rxMethod<void>((input$) =>
      input$.pipe(
    ...
    const getWarehouseCardsSilently = rxMethod<void>((input$) =>
      input$.pipe(
        switchMap((_) => {
          const dest = store.destination();
          const month = store.monthYear();
    ...
    const createUpdateWarehouseCard = rxMethod<{
      card: WarehouseCard;
      message: string;
    }>((input$) =>
      input$.pipe(
        tap((_) => patchState(store, toggleIsSaving())),
        switchMap(({ card, message }) =>
          store._warehouseService.createUpdateWarehouseCard(card).pipe(
            handleApiResponse(toaster, {
              ...
              onSuccess: async (card) => {
                ...
                const messageToSend = message
                  ? `${getMessageTime()} ${user.name}: ${message}`
                  : `${getMessageTime()} ${user.name}: aktualizován sklad!`;
                store.signalR.sendWarehouseCards(
                  user.name,
                  store.destination(),
                  [card],
                  messageToSend,
    ...
    const deleteCards = rxMethod<void>((input$) =>
      input$.pipe(
        tap((_) => patchState(store, toggleIsDeleting())),
        switchMap((_) =>
          store._warehouseService
            .deleteWarehouseCards(store.monthYear(), store.destination())
            .pipe(
              handleApiResponse(toaster, {
    ...
    const deleteCard = rxMethod<void>((input$) =>
      input$.pipe(
        ...
        switchMap((_) =>
          store._warehouseService
            .deleteWarehouseCard(store.selectedCard()?.id || 0)
            .pipe(
              handleApiResponse(toaster, {
                ...
                onSuccess: (cards) => {
                  ...
                  store.signalR.sendWarehouseCards(
                    user.name,
                    store.selectedCard()!.destination,
                    cards,
                    messageToSend,
                  );
                },
    ...
    const getWarehouseItems = rxMethod<void>((input$) =>
      input$.pipe(
        tap((_) => patchState(store, toggleIsLoading())),
        switchMap((_) =>
          store._warehouseService.getAllWarehouseItems().pipe(
            handleApiResponse(toaster, {
              onSuccess: (items) => {
    ...
    const deleteWarehouseItem = rxMethod<WarehouseItem>((input$) =>
      input$.pipe(
        tap((_) => patchState(store, toggleIsLoading())),
        switchMap((warehouseItem) =>
          store._warehouseService.deleteWarehouseItem(warehouseItem.id).pipe(
            handleApiResponse(toaster, {
              ...
              onSuccess: (items) => {
                ...
                store.signalR.sendWarehouseItems(
                  user.name,
                  items,
                  messageToSend,
                );
    ...
    const createPdf = rxMethod<void>((input$) =>
      input$.pipe(
        tap((_) => patchState(store, toggleIsPdfLoading())),
        switchMap((_) =>
          store._warehouseService.createPDF(store.cards()).pipe(
            handleApiResponse(toaster, {
              onSuccess: (file) => {
                downloadPdf(
                  file,
                  `Sklad - ${store.cards()[0].monthYearName} - ${store.cards()[0].destination}.pdf`,
                );
    ...
    const createWarehouseItem = rxMethod<WarehouseItem>((input$) =>
      input$.pipe(
        tap((_) => patchState(store, toggleIsSaving())),
        switchMap((item) =>
          store._warehouseService.createWarehouseItem(item).pipe(
            handleApiResponse(toaster, {
    ...
    const updateWarehouseItem = rxMethod<{
      newItem: WarehouseItem;
      oldName: string;
    }>((input$) =>
      input$.pipe(
        tap((_) => patchState(store, toggleIsSaving())),
        switchMap(({ newItem, oldName }) =>
          store._warehouseService.updateWarehouseItem(newItem).pipe(
            handleApiResponse(toaster, {
    ...
    const removeWarehouseItem = (warehouseItem: WarehouseItem) => {
      ...
      warehouseItems = warehouseItems.filter(
        (i) => i.position !== warehouseItem.position,
      );
    ...
    return {
      ...
      resetMonthYaer: () =>
        setMonthYear(
          MONTHS_NUM[new Date().getMonth()] + new Date().getFullYear(),
        ),
      ...
      setWarehouseNave: (warehouseNav: 'units' | 'board' | 'items') =>
        patchState(store, { warehouseNav }),
      slideTo,
      consumeSlideToIndex,
      removeWarehouseItem: (warehouseItem: WarehouseItem) =>
        removeWarehouseItem(warehouseItem),
      reorderWarehouseItems: (items: WarehouseItem[]) =>
        reorderWarehouseItems(items),
      createPdf: () => createPdf(),
      createWarehouseItem: (newItem: WarehouseItem) =>
        createWarehouseItem(newItem),
      updateWarehouseItem: (newItem: WarehouseItem, oldName: string) =>
        updateWarehouseItem({ newItem, oldName }),
      createUpdateWarehouseCard: (card: WarehouseCard, message: string) =>
        createUpdateWarehouseCard({ card, message }),
      setSelectedUnit: (selectedUnit: WarehouseUnit) =>
        patchState(store, { selectedUnit }),
    ...
    onInit(store) {
      effect(() => {
        ...
        if (received && received.length > 0) {
          if (
            (store._router.url === '/warehouse' ||
              store._router.url === '/warehouse-units') &&
            store.monthYear() === received[0].monthYear
          ) {
      ...
      effect(() => {
        ...
        if (received && received.length > 0) {
          if (
            ...
            store._router.url === '/items'
          ) {
            patchState(store, { warehouseItems: received });
          }
          if (
            store.auth.user()?.role === 'Master' &&
            store._router.url === '/warehouse'
          ) {
