...
export class UserComponent {
  ...
  protected form = form(this.model, (s) => {
    ...
    validate(s.destinations, ({ value }) =>
      atLeastOnePositionSelected(value()),
    );

    validate(
      s.name,
      maxLenValidator(this.USER_MAX_NAME, `Max ${this.USER_MAX_NAME} znaků`),
    );
    validate(
      s.surname,
      maxLenValidator(
        this.USER_MAX_SURNAME,
        `Max ${this.USER_MAX_SURNAME} znaků`,
      ),
    );
    validate(
      s.password,
      maxLenValidator(
        this.USER_MAX_PASSWORD,
        `Max ${this.USER_MAX_PASSWORD} znaků`,
      ),
    );
  });
  ...
  onFileSelected(event: Event) {
    ...
    if (!allowedTypes.has(file.type)) {
      this.usersStore.error(
        'Nepovolený formát! Povoleny jsou pouze JPEG, JPG nebo PNG.',
      );
    ...
    if (file.size > 1 * 1024 * 1024) {
      this.usersStore.error(
        'Soubor je příliš velký! Maximální velikost je 1 MB.',
      );
      this.imagePicker()!.nativeElement.value = '';
      return;
    }
  ...
  private getUserDestination(user: User, destination: Destination) {
    ...
    return DESTINATIONS.map((destination) => {
      ...
      return {
        ...
        positions: POSITIONS.filter((position) => {
          ...
          const existingPosition = existing.positions?.find(
  ...
  private imageChangeIntent = computed(() => {
    ...
    if (this.originalImageName() && !this.selectedImageName()) {
      return 'removed';
    }
    return 'unchanged';
  });
